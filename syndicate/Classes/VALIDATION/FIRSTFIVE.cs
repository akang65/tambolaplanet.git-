...
namespace syndicate.Classes.VALIDATION
{
    ...
    public class FIRSTFIVE
    {
        ...
        public List<string> getValidationTicketsAndValidate()
        {
            using (IDbConnection connection = new SqlConnection(Helper.CnnVal("TambolaStars")))
            {
                ...
                for (int k = 1; k <= ticketcount.Length; k++)
                {
                    ...
                    var data = connection.Query<firstFive>("dbo.sp_getValidationTickets @TicketName", new { TicketName = ticketName }).ToList();
                    ...
                    if (countexist >= 5)
                        {
                            ...
                            //M++;
                            AddFirstFiveWinner(ticketName,countRandom);
                        }
                        //else
                        //{
                        //    int M = k - 1;
        ...
        public void AddFirstFiveWinner(string ticketNo,int serialNo)
        {
            using (IDbConnection connection = new SqlConnection(Helper.CnnVal("TambolaStars")))
            {

                connection.Execute("dbo.sp_AddFirstFiveWinner @TicketName,@SerialNo", new { TicketName = ticketNo, SerialNo=serialNo });
